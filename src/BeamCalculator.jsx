...
const DEFAULT_POINT_LOADS = [{ id: 1, positionMm: 1500, magnitudeKn: 8 }];
...
const buildNodes = ({ lengthMm, supports, pointLoads, udls }) => {
...
const mapLoadsToNodes = (nodes, pointLoads) => {
...
const getElementUdl = (udls, xStart, xEnd) => {
...
const applyBoundaryConditions = (nodes, K, F) => {
  const constrained = [];
...
const computeDiagrams = (nodes, elementData, displacements) => {
  const samples = [];
...
const scalePoints = (data, yKey, width, height) => {
...
const BeamCalculator = () => {
  ...
  const [pointLoads, setPointLoads] = useState(DEFAULT_POINT_LOADS);
  const [udls, setUdls] = useState(DEFAULT_UDLS);
  ...
  const analysis = useMemo(() => {
    ...
    const { constrained, free, Kff, Ff } = applyBoundaryConditions(nodes, K, F);
  ...
  const handlePointLoadChange = (id, field, value) => {
    setPointLoads((prev) =>
      prev.map((load) => (load.id === id ? { ...load, [field]: value } : load))
    );
  };

  const handleUdlChange = (id, field, value) => {
    setUdls((prev) =>
